...
public class CorrelationMatrixGraph extends JPanel implements MouseListener, MouseMotionListener
{
	...
	private final float LABEL_WIDTH = 0.7f;
	...
	protected void paintComponent(Graphics g)
	{
		...
		double labelMargin = (1 - LABEL_WIDTH) * cellSize / 2;

		g2d.setFont(matrix.getFont().deriveFont((float) (cellSize * LABEL_WIDTH)));
		titlesCellWidth = getLabelsWidth(matrix.getTitles(), g2d) + labelMargin * 2;
	...
	private void paintCell(Graphics2D g2d, Cell cell)
	{
		...
		double margin =  ((cellSize - cellSize * CIRCLE_WIDTH) / 2);
	...
	private Zoom createZoom(Graphics2D g2d, int i, int j)
	{
		...
		zoom.labelsMargin = cellSize * (1 - LABEL_WIDTH) / 2;
		...
		zoom.horizontalLabelsWidth = getLabelsWidth(zoom.horizontalLabels, g2d) + zoom.cellSize * (1 - LABEL_WIDTH);

		zoom.verticalLabels = matrix.getTitles().subList(zoom.startI, zoom.startI + zoom.length);
		zoom.verticalLabelsWidth = getLabelsWidth(zoom.verticalLabels, g2d) + zoom.cellSize * (1 - LABEL_WIDTH);
	...
	public String getToolTipText(MouseEvent event)
	{
		...
		if (cell != null)
		{
			...
			if (dataTypeI == CellType.NUMERIC && (dataTypeJ == CellType.NUMERIC))
			{  // Numeric vs. Numeric

				text += "Pearson's R\u00B2 = " + formatValue(matrix.getDataSqr()[i][j]) + "<br/>";
				text += "Pearson's R = " + formatValue(matrix.getData()[i][j]);
			}
			else if (dataTypeI == CellType.NOMINAL && dataTypeJ == CellType.NOMINAL)
			{ // Nominal vs. Nominal
